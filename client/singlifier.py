...
class PageSinglifier():
    ...
    def _take_libs(self):
        ...
        for filename in os.listdir(js_libs_folder):
            with open(path.join(js_libs_folder, filename), 'r+', encoding="utf8") as file:
                js_libs[filename[:-3]] = file.read()
        for filename in os.listdir(css_libs_folder):
            with open(path.join(css_libs_folder, filename), 'r+', encoding="utf8") as file:
                css_libs[filename[:-4]] = file.read()
        for filename in os.listdir(html_libs_folder):
            with open(path.join(html_libs_folder, filename), 'r+', encoding="utf8") as file:
    ...
    def run(self):
        ...
        for page_folder_name in os.listdir(pages_folder):
            ...
            with open(css_file_path, 'r', encoding="utf8") as css_file:
                css = css_file.read()
            ...
            for imagename in os.listdir(images_folder):
                ...
                with open(imagepath, 'r', encoding="utf8") as imagefile:
